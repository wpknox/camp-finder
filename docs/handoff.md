...
### Done 2026-07-04 (later session) — moderation polish IMPLEMENTED
Both pending items below are now built on `feat/admin-moderation` (plan: `docs/plans/2026-07-04-moderation-polish.md`, executed via subagents, reviewed by orchestrator):
- **Field-level merge chooser**: `mergeFacilityFields(winner, loser, choices?)` accepts per-field `'winner'|'loser'` selections (`name`, `location` = lat+lng as a unit, `fcfs` counts as a unit, plus the scalar fields); explicit choice wins even over empty values, unpicked fields keep the old gap-fill. API validates `field_choices` (400 on bad keys/values) and now writes `name`/`lat`/`lng` in the winner edit; GET returns full `facility_a_data`/`facility_b_data`. Admin UI is a side-by-side A|B grid with master "use all of A/B" + per-field overrides; `field_choices` is only sent when the admin actually engaged the grid, so a quick default approve keeps the legacy gap-fill. 14 engine tests (9 new).
- **Detail-panel fresh fetch**: opening (or re-opening) the detail panel fetches `/api/facilities/{id}` fresh — approved edits/merges now show without "Search this area"; 404 (merged-away facility) closes the panel.
- App-review notes for the user: `docs/review-notes-2026-07-04.md` (merge irreversibility, fetch-all scaling, TB workaround accumulation, WHERE-interpolation footgun, `{@html}` note, UI ideas).
- CLAUDE.md refreshed (stale "planned"/"open questions" sections removed, design-language pointer added).
- **Not yet done:** live browser test of the new merge UI; merge of `feat/admin-moderation` → `main`.

### Prior pending-work notes (now implemented, kept for context)
