...
export function extractFees(feeDescription: string): {
  fee_min: number | null;
  fee_max: number | null;
} {
  ...
  const dollars = [...feeDescription.matchAll(/\$(\d+(?:\.\d+)?)/g)].map((m) =>
...
export function extractFeesFromDescription(description: string): {
  fee_min: number | null;
  fee_max: number | null;
} {
